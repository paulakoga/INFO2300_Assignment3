...
export class ConcertListComponent implements AfterViewInit {
  ...
  getDate(datetime: Date) {
    ...
    let stringdate: string = date.getFullYear() + "-" + (date.getMonth() + 1).toLocaleString('en-US', {
  ...
  getSetlistName(setlistId: number){
    let setlistName: string | undefined = (this.datastore.getSetlist().find(d => d.id == setlistId))?.name
    return setlistName;
  }
